...
const OAUTH_TENANT_COOKIE = "google_drive_oauth_tenant_id";
...
export async function GET(request: NextRequest) {
  ...
  const errorRedirect = NextResponse.redirect(new URL("/dashboard?drive=error", request.url));
  ...
  try {
    const token = await exchangeGoogleAuthorizationCode({
      code,
      redirectUri: getGoogleRedirectUri(IntegrationProvider.GOOGLE_DRIVE, request.nextUrl.origin)
    });
    ...
    const successRedirect = NextResponse.redirect(new URL("/dashboard?drive=connected", request.url));
